...
API_KEY = "pub_fee2cff81e04f70c2660ce74023dc411"
...
class TestGetRequests:
    ...
    def test_get_user_list_returns_200(self):
        ...
        response = requests.get(f"{BASE}/users?page=1", headers=HEADERS)
    ...
    def test_get_single_user_returns_correct_id(self):
        ...
        response = requests.get(f"{BASE}/users/2", headers=HEADERS)
        assert response.status_code == 200
    ...
    def test_get_nonexistent_user_returns_404(self):
        ...
        response = requests.get(f"{BASE}/users/9999", headers=HEADERS)
...
class TestPostRequests:
    ...
    def test_create_user_returns_201(self):
        ...
        response = requests.post(f"{BASE}/users", json=payload, headers=HEADERS)
...
class TestPutAndDelete:
    ...
    def test_update_user_returns_200(self):
        ...
        response = requests.put(f"{BASE}/users/2", json=payload, headers=HEADERS)
        assert response.status_code == 200
        body = response.json()
    ...
    def test_delete_user_returns_204(self):
        ...
        response = requests.delete(f"{BASE}/users/2", headers=HEADERS)
...
class TestChainedAPIFlow:
    ...
    def test_create_then_verify_user(self):
        ...
        create_resp = requests.post(f"{BASE}/users", json=payload, headers=HEADERS)
        ...
        update_resp = requests.put(f"{BASE}/users/{created_id}", json=update_payload, headers=HEADERS)
        assert update_resp.status_code == 200
        assert update_resp.json()["job"] == "Senior QA Tester"

        # Step 4: Delete
        delete_resp = requests.delete(f"{BASE}/users/{created_id}", headers=HEADERS)
